...
const Sign_in = () => {
  ...
  const sendData = async (event) => {
    ...
    try {
      ...
      toast.warn(error.message, { position: 'bottom-center' });
    }
  };
  ...
  return (
    <>
      <section className=" relative max-h-screen">
        <img className="fixed mb-[-200px] w-full h-full bg-cover blur-sm" src={hero} alt="Background" />
        <div className="relative z-10 w-[85%] mx-auto flex flex-col items-center">
          <div className="index">
            <h1 className="w-[200px] mt-10 ml-[6rem] cursor-pointer text-black">Radiant Life</h1>
          </div><br/>
          <div className="borrder-none p-2.5 mt-[-50px] mb-2.5 rounded-md shadow-md">
            <form method="POST">
              <h2 className="font-medium text-[27px] my-2.5 pl-[5.5rem] pt-4">Sign-In</h2>
              <div className="form-data">
                <label className="text-[13px] font-bold mb-1" htmlFor="email">Email</label>
                <input
                  className="placeholder-black w-[95%] h-[31px] p-[3px_7px] rounded-sm border border-gray-200 bg-transparent mb-2.5 mt-1 outline-none focus:shadow-outline"
                  type="text"
                  name="email"
              ...
              <NavLink to="/reset" className="text-blue-500">Forgot Password</NavLink>
          ...
          <div className="create-accountinfo">
            <p className="text-[12px] text-center text-black-400 mt-5 font-semibold">New To Radiant Life?</p>
            <NavLink to="/register">
              <button className="bg-gray-300 text-black  px-1 py-1 rounded-md mt-2">Create Your Radiant Life Account</button>
            </NavLink>
          </div>
          <ToastContainer />
        </div>
